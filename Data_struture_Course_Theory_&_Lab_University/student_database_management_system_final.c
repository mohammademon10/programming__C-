...
int main()
{
    ...
    while (1)
    {
        ...
        case 6: // Search option
            system("cls");
            search_student(head); // Call the separate search function
            break;
        case 0:
...
Node *update_student_info(Node *head)
{
    ...
    printf("\t\t\t\tStudent found. You can update the following details:\n");

    // Update Name
    printf("\t\t\t\tCurrent Name: %s\n", temp->info.name);
    printf("\t\t\t\tEnter new name (or press Enter to keep it unchanged): ");
    fgets(temp->info.name, sizeof(temp->info.name), stdin);
    ...
    printf("\t\t\t\tEnter new email (or press Enter to keep it unchanged): ");
    fgets(newEmail, sizeof(newEmail), stdin);
    newEmail[strcspn(newEmail, "\n")] = '\0';
    if (strlen(newEmail) > 0)
    {
        while (strstr(newEmail, validDomain) == NULL || isDuplicateEmail(head, newEmail))
        {
            if (strstr(newEmail, validDomain) == NULL)
            {
                printf("\t\t\t\tInvalid email! Must contain '@diu.edu.bd'. Enter again: ");
            }
            else
        ...
        strcpy(temp->info.email, newEmail);
    }

    // Update Cell with validation
    char newCell[20];
    printf("\t\t\t\tCurrent Cell: %s\n", temp->info.cell);
    printf("\t\t\t\tEnter new contact number (or press Enter to keep it unchanged): ");
...
Node *search_student_by_id(Node *head, char *id)
{
    while (head != NULL)
    {
        if (strcmp(head->info.id, id) == 0)//0 if both strings are identical.
        {
            return head;
        }
        head = head->next;
...
void search_student(Node *head)
{
    ...
    Node *result = search_student_by_id(head, id);
    if (result != NULL)
    {
        printf("\t\t\t\tStudent found!\n");
        student_details(result->info);
    }
    else
    {
        printf("\t\t\t\tNo student found with this ID.\n");
